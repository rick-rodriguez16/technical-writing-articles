...
| `index`     | (optional parameter) The position that the current element is located in `myArray`.         |
| `array`     | (optional parameter) The array that the `some()` method owas used on (in this case, `myArray`).|
| `thisArg`   | (optional parameter) Used as the `this` value when calling the `callbackFunction`. Defaults to undefined when not used.                                                               |
